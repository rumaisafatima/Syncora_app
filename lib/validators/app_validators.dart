...
class AppValidators {
  ...
  /// Validates a first name (letters only).
  static String? firstName(String? value) {
    final requiredVal = AppValidators.required(value, fieldName: 'First name');
    if (requiredVal != null) return requiredVal;

    final trimmed = value!.trim();
    if (!RegExp(r"^[a-zA-Z'-]+$").hasMatch(trimmed)) {
      return 'First name may only contain letters, spaces, hyphens, or apostrophes';
    }
    return null;
  }

  /// Validates a last name (letters only).
  static String? lastName(String? value) {
    final requiredVal = AppValidators.required(value, fieldName: 'Last name');
    if (requiredVal != null) return requiredVal;

    final trimmed = value!.trim();
    if (!RegExp(r"^[a-zA-Z'-]+$").hasMatch(trimmed)) {
      return 'Last name may only contain letters, spaces, hyphens, or apostrophes';
    }
    return null;
  }
